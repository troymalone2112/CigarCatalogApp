...
date: entry.smoking_date || entry.date ? fromLocalDateString(entry.smoking_date || entry.date) : new Date()
```
...
const invalidDates = sortedEntries.filter(entry => 
  !entry.date || !(entry.date instanceof Date) || isNaN(entry.date.getTime())
);
