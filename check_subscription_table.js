...
async function checkSubscriptionTable() {
  try {
    ...
    const expectedFields = [
      'user_id', 'plan_id', 'status', 'is_premium', 'trial_start_date', 'trial_end_date',
      'subscription_start_date', 'subscription_end_date', 'auto_renew', 'created_at', 'updated_at'
    ];
    ...
    if (subscription.status === 'trial' && !subscription.is_premium) {
      ...
      console.log(`   Actual: status='${subscription.status}', is_premium=${subscription.is_premium}`);
    }
