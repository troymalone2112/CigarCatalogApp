-- Database Optimization Script
-- Based on Supabase linter recommendations

-- ============================================
-- 1. ADD MISSING FOREIGN KEY INDEXES (CRITICAL)
-- ============================================

-- Add index for recommendations.cigar_id foreign key
CREATE INDEX IF NOT EXISTS idx_recommendations_cigar_id ON recommendations(cigar_id);

-- Add index for user_subscriptions.plan_id foreign key  
CREATE INDEX IF NOT EXISTS idx_user_subscriptions_plan_id ON user_subscriptions(plan_id);

-- ============================================
-- 2. REMOVE UNUSED INDEXES (PERFORMANCE CLEANUP)
-- ============================================

-- Remove unused journal_entries indexes
DROP INDEX IF EXISTS idx_journal_entries_selected_flavors;
DROP INDEX IF EXISTS idx_journal_entries_cigar_data;

-- Remove unused inventory indexes
DROP INDEX IF EXISTS idx_inventory_date_acquired;
DROP INDEX IF EXISTS idx_inventory_aging_preference;
DROP INDEX IF EXISTS idx_inventory_cigar_line;
DROP INDEX IF EXISTS idx_inventory_cigar_brand;

-- Remove unused usage_tracking indexes
DROP INDEX IF EXISTS idx_usage_tracking_action;

-- ============================================
-- 3. KEEP IMPORTANT INDEXES (DO NOT REMOVE)
-- ============================================

-- These indexes should be KEPT as they are used:
-- - idx_journal_entries_photos (for image queries)
-- - idx_journal_entries_created_at (for date sorting)
-- - idx_journal_entries_image_url (for image queries)
-- - idx_user_subscriptions_revenuecat_user_id (for RevenueCat)
-- - idx_user_subscriptions_status (for subscription checks)
-- - idx_user_subscriptions_trial_end (for trial expiration)

-- ============================================
-- 4. ADD OPTIMIZED INDEXES FOR COMMON QUERIES
-- ============================================

-- Add composite index for journal entries (user_id + created_at)
CREATE INDEX IF NOT EXISTS idx_journal_entries_user_created 
ON journal_entries(user_id, created_at DESC);

-- Add composite index for inventory (user_id + humidor_id)
CREATE INDEX IF NOT EXISTS idx_inventory_user_humidor 
ON inventory(user_id, humidor_id);

-- Add composite index for humidors (user_id + created_at)
CREATE INDEX IF NOT EXISTS idx_humidors_user_created 
ON humidors(user_id, created_at DESC);

-- ============================================
-- 5. ANALYZE TABLES FOR BETTER QUERY PLANNING
-- ============================================

-- Update table statistics for better query planning
ANALYZE journal_entries;
ANALYZE inventory;
ANALYZE humidors;
ANALYZE user_subscriptions;
ANALYZE recommendations;
ANALYZE usage_tracking;



