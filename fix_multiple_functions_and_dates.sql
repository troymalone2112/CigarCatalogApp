-- Fix Multiple Function Versions Issue
-- This script removes all existing versions of handle_revenuecat_webhook and creates a clean version
...
BEGIN
    ...
    FOR func_record IN
        ...
        WHERE p.proname = 'handle_revenuecat_webhook'
        AND n.nspname = 'public'
    LOOP
        RAISE NOTICE 'Dropping function: %.%(%)', 
    ...
    RAISE NOTICE 'All existing handle_revenuecat_webhook functions removed';
END $$;

-- Step 2: Create the corrected function with proper date validation and RevenueCat user ID
...
-- Step 4: Create webhook events table for debugging (if it doesn't exist)
...
-- Step 9: Final verification - show function signature
DO $$
DECLARE
    func_record RECORD;
BEGIN
    ...
    FOR func_record IN
        ...
        AND n.nspname = 'public'
    LOOP
        RAISE NOTICE 'Function exists: %(%)', func_record.function_name, func_record.arguments;
    END LOOP;
END $$;
