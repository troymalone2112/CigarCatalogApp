-- Simple fix for trial dates - use UTC consistently
-- Run this in Supabase SQL Editor

-- Drop and recreate the trigger function with simplified date handling
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS public.handle_new_user();

-- Create the updated function with simplified timezone handling
...
BEGIN
  ...
  -- Use UTC for consistent trial tracking
  -- Trial starts immediately in UTC
  trial_start := NOW() AT TIME ZONE 'UTC';
  -- Trial ends 3 days from now in UTC
  trial_end := (NOW() AT TIME ZONE 'UTC') + INTERVAL '3 days';
  ...
  RAISE NOTICE 'Trial start (UTC): %, Trial end (UTC): %', trial_start, trial_end;
