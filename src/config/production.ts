 ...
 */
import { getTimeoutValue, DEVELOPMENT_CONFIG } from './development';
...
export const getProductionTimeoutValue = (
  ...
  if (isProduction()) {
    return (
      PRODUCTION_CONFIG.TIMEOUTS[type.toUpperCase() as keyof typeof PRODUCTION_CONFIG.TIMEOUTS] ||
      baseTimeout
    );
  }
...
export const getProductionRetryDelay = (attempt: number, baseDelay: number = 500): number => {
  const config = isProduction() ? PRODUCTION_CONFIG.RETRIES : DEVELOPMENT_CONFIG.RETRIES;
