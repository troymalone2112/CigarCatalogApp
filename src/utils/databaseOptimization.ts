...
export class DatabaseOptimization {
  ...
  static async executeWithRetry<T>(
    ...
    const { timeout = 20000, retries = 3, retryDelay = 1000, operation: operationName } = options;
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        console.log(
          `🔄 Database operation: ${operationName} (attempt ${attempt + 1}/${retries + 1})`,
        );
        ...
        console.warn(
          `⚠️ Database operation: ${operationName} failed (attempt ${attempt + 1}):`,
          error,
        );
